...
```
顺便说一下，这个uri是咱自己摸索出来的，一次成功，重启openresty再访问不存在的页面：`http://192.168.1.2/item-100.html?id=50` 则打印出了：
```
uri:/item-100.html arg:50
```
太高兴啦😊

高兴完了，冷静下来总结一下：error_page再出现了404错误之后，会转到一个叫"/404.html"的location，这里转的时候，把报404的这个页面的uri和参数（$arg_id&uri=$uri）取到，并且都转交给那个新的location。到此为止，哪个uri什么参数报错了。  
接下来用`local id = string.sub(uri, 7, -6)`截取item的id，因为uri长这样：`/item-100.html`,所以从第7个字符开始（lua字符串下标从1开始，而不是0）截取到倒数第6个。然后就可以拿着它去数据库找了：
```
# 遇到404 错误就转到一个叫"/404.html"的location 
error_page  404              /404.html?id=$arg_id&uri=$uri;
location = /404.html {
    default_type text/html;
    content_by_lua '
...
```
这里还不算完，还可以继续优化：DB换成Redis、nginx本地存缓存、缓存预热、nginx优化（worker）Linux优化等等
